...
def tryrec(userid):
    ...
    try:
        ...
        for genid in genids:
            ...
            if(Runtime == 1):
                if(Year == 1):
                    state3 = "SELECT Name, MovieID,Rating FROM Movie1 m WHERE (m.Runtime <= 60 )AND (2010 < m.Year <=2016) AND m.MovieID IN (SELECT MovieID FROM MovieGenre1 WHERE GenreID = %d) AND (m.MovieID not IN(SELECT MovieID FROM UserSeen WHERE UserID = %d))ORDER BY Rating DESC LIMIT 3" %genid, userid                                                                                   userid
                elif(Year == 2):
...
def userclick(userid,movieid):
    state1 = "SELECT GenreID FROM Movie1 WHERE MovieID = %d" %movieid
    try:
        x = cursor.execute(state1)
        if x == 0:
            return None
        ...
        state2 = "SELECT UserID FROM MovieGenre1 WHERE GenreID = %d and UserID = %d" %genid,userid
        x= cursor.execute(state2)
        ##check whether we have this genre for user already, if not, create a new genre for him or her
        if x== 0:
            state3 = "INSERT INTO UserGenre(UserID,GenreID,Clicktimes) VALUES(%d,%d,1)" % userid, genid
        else:
            state3 = "UPDATE UserGenre SET Clicktimes = Clicktimes +1 WHERE GenreID = %d and UserID = %d " %genid,userid
