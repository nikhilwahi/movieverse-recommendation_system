...
def main_function(set_val, user_ratings, age, gender, occupation, ask_vals, user_id):
	user = []
	item = []
	...
	sample_movies = []
	for movie in ask:
		sample_movies.append(movie.title)

	if(set_val == 0):
		return sample_movies, ask

	for i, movie in enumerate(ask):
		a = user_ratings[i]
